...
class ConnectionManager:
    ...
    async def send_location_update(self, task_id: str, data: dict):
        if task_id in self.active_connections:
            try:
...
@app.websocket("/ws/tracking/{task_id}")
...
async def update_tracking_location(
    ...
    # Send update via WebSocket to connected client
    await manager.send_location_update(task_id, {
        "latitude": latitude,
        "longitude": longitude,
